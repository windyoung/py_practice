...
SELECT L.PROD_INST_ID, L.ACC_NUM
 ...
 WHERE L.PROD_INST_ID IN
       (SELECT M1.PROD_INST_ID
          FROM (SELECT (SELECT NVL(SUM(NVL(H1.ORI_CHARGE, 0) +
                                       NVL(H1.SETT_CHARGE, 0)),
                                   0)
                          FROM ABC.RM_DOC@TO_ABC H1
                         WHERE H1.ACCT_ID = E.ACCT_ID
                           AND H1.DUE_DATE < SYSDATE
                           AND H1.DOC_TYPE_ID = 1) AS ALL_DUE,
                       (SELECT NVL(SUM(NVL(H2.KIV_AMOUNT, 0) +
                                       NVL(H2.KIV_SETTLE_AMOUNT, 0)),
                                   0)
                          FROM ABC.COLL_KIV_LIST@TO_ABC H2
                         WHERE H2.ACCT_ID = E.ACCT_ID
                           AND H2.EXP_DATE > SYSDATE) AS ALL_KIV_AMOUNT,
                       (SELECT NVL(SUM(NVL(H3.DUE_AMOUNT, 0) +
                                       NVL(H3.PAYEMTN_AMOUNT, 0)),
                                   0)
                          FROM ABC.COLL_PAYMENT_PLAN@TO_ABC H3
                         WHERE H3.ACCT_ID = E.ACCT_ID
                           AND H3.PLAN_STATUS = 'A'
                           AND H3.DUE_DATE > SYSDATE) AS INSTALLMENT_AMOUNT,
                       (SELECT NVL(SUM(NVL(H4.AMOUNT, 0)), 0)
                          FROM ABC.RM_MAIN_BAL@TO_ABC H4
                         WHERE H4.ACCT_ID = E.ACCT_ID) AS ADVANCE_PAYMENT,
                       C.STOP_TYPE,
                       E.*
                  FROM (SELECT A.*
                         ...
                         WHERE A.STOP_TYPE = '130001'
                           AND NOT EXISTS
                         (SELECT 1
                                  FROM COC.PROD_INST_ACCT_REL B
                                 WHERE A.PROD_INST_ID = B.PROD_INST_ID
                                   AND B.IF_DEFAULT_ACCT_ID <> 1)) C
                  LEFT JOIN COC.PROD_INST_ACCT_REL E
                    ON C.PROD_INST_ID = E.PROD_INST_ID) M1
         WHERE M1.ALL_DUE - M1.ALL_KIV_AMOUNT - M1.INSTALLMENT_AMOUNT +
               M1.ADVANCE_PAYMENT <= 0
           AND EXISTS (SELECT 1
                  FROM COC.PROD_INST P
                 WHERE M1.PROD_INST_ID = P.PROD_INST_ID
