...
def test4_resp2yaml(resp):
    ...
    res = pd.DataFrame.from_dict(xml_data['soapenv:Envelope']['soapenv:Body']).to_json(
    ).replace('ns:', '').replace('ax21:', '')
...
def test5_getws_resp_createnewcust(url, msg):
   #  url = 'http://172.16.24.191:9902/DbepService?wsdl'
    clt = Client(url)
    clt.service.createCustomer(msg)
   #  print(clt.service.)
    req = str(clt.last_sent())
    resp = str(clt.last_received())
    # print(req)
...
def test5_getws_resp_qryCustomerBasicInfo(url, msg):
   #  url = 'http://172.16.24.191:9902/DbepService?wsdl'
    clt = Client(url)
    clt.service.qryCustomerDetail(msg)
   #  print(clt.service.)
    req = str(clt.last_sent())
    resp = str(clt.last_received())
    # print(req)
...
custmsg = {"birthday": "14.05.2000 14:50:17",
           "certNbr": "1234567890",
           "certType": "123",
           "cityCode": "qwe",
           "contactInfoDtoList": {"contactGender": "M", 
...
# a=test5_getws_resp_createnewcust(ws_url, custmsg)
# print(a)
custaaa={'certNum':'','certType':'','custNumber':'zhang'}
aaa=test5_getws_resp_qryCustomerBasicInfo(ws_url, custaaa)
print(aaa)
